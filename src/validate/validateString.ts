...
const validateDateParts = (year: string, month: string, day: string, path: string[], issues: Issue<string[]>[]) => {
    const yearInt = parseInt(year, 10);
    const monthInt = parseInt(month, 10);
    const dayInt = parseInt(day, 10);

    if (monthInt < 1 || monthInt > 12) {
        issues.push({ on: path, message: 'value must have month between 01 and 12' });
    }
    else {
        ...
        if (dayInt < 1 || dayInt > monthDays)
            issues.push({ on: path, message: `value must have day between 01 and ${monthDays}}` });
    }
...
const validateTimeParts = (
    ...
    path: string[], issues: Issue<string[]>[]
) => {
    ...
    if (hourInt < 0 || hourInt > 23)
        issues.push({ on: path, message: 'value must have hours between 00 and 23' });
    if (minuteInt < 0 || minuteInt > 59)
        issues.push({ on: path, message: 'value must have minutes between 00 and 59' });
    if (secondInt < 0 || secondInt > 59)
        issues.push({ on: path, message: 'value must have seconds between 00 and 59' });
    if (millisecondInt < 0 || millisecondInt > 999)
        issues.push({ on: path, message: 'value must have milliseconds between 00 and 99' });
    return issues;
};

const tzRegex = /([+-])(\d{2}):(\d{2})/;

const validateTimeZone = (part: string, path: string[], issues: Issue<string[]>[]) => {
    if (part !== 'Z') {
        ...
        if (hoursInt > 23)
            issues.push({ on: path, message: 'value must have timezone hours between 00 and 23' });
        else if (minutesInt > 59) {
            issues.push({ on: path, message: 'value must have timezone minutes between 00 and 59' });
        }
    };
...
const dateTester: StringTester = (value, path) => {
    ...
    if (!match)
        return [{ on: path, message: 'value must be in date format YYYY-MM-DD' }];
...
const timeTester: StringTester = (value, path) => {
    ...
    if (!match)
        return [{ on: path, message: 'value must be in time format HH:mm:ss(.nnn)' }];
...
const datetimeTester: StringTester = (value, path) => {
    ...
    if (!match)
        return [{ on: path, message: 'value must be in datetime format HH:mm:ss(.nnn)Thh:MM:SS(.nnn)(+-hh(:mm)|Z)' }];
...
const uuidTester: StringTester = (value, path) => {
    return uuidRegex.test(value) ? true : [{ on: path, message: 'value must be a valid uuid' }];
};

const base64Regex = /^(?:[A-Za-z0-9+/]{4})*(?:[A-Za-z0-9+/]{2}==|[A-Za-z0-9+/]{3}=)?$/;

const base64Tester: StringTester = (value, path) => {
    return base64Regex.test(value) ? true : [{ on: path, message: 'value must be valid base64 encoded string' }];
};

const emailRegex = /^(?:[a-z0-9!#$%&'*+/=?^_`{|}~-]+(?:\.[a-z0-9!#$%&'*+/=?^_`{|}~-]+)*|"(?:[\x01-\x08\x0b\x0c\x0e-\x1f\x21\x23-\x5b\x5d-\x7f]|\\[\x01-\x09\x0b\x0c\x0e-\x7f])*")@(?:(?:[a-z0-9](?:[a-z0-9-]*[a-z0-9])?\.)+[a-z0-9](?:[a-z0-9-]*[a-z0-9])?|\[(?:(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.){3}(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?|[a-z0-9-]*[a-z0-9]:(?:[\x01-\x08\x0b\x0c\x0e-\x1f\x21-\x5a\x53-\x7f]|\\[\x01-\x09\x0b\x0c\x0e-\x7f])+)\])$/i;

const emailTester: StringTester = (value, path) => {
    return emailRegex.test(value) ? true : [{ on: path, message: 'value must be a valid email' }];
};
...
export const validateString = (value: any, schema: StringSchema, path: string[], prefix: string = 'value'): ValidationResult => {
    if (typeof value !== 'string') return [{ on: path, message: `${prefix} must be string.` }];
    ...
    if (filter) {
        if (typeof filter === 'string') {
            if (isStringFormat(filter)) {
                ...
                if (regex) {
                    if (!regex.test(value))
                        return [{ on: path, message: `${prefix}  must match custom regex: ${regex}` }];
                }
                else {
                    return [{ on: path, message: `invalid ${prefix} string filter: ${filter}` }];
                }
            }
        }
        else if (filter instanceof RegExp) {
            if (!filter.test(value))
                return [{ on: path, message: `${prefix}  must match custom regex: ${filter}` }];
